...
export const productionOrderStatusEnum = pgEnum("production_order_status", [
  ...
  // 2026-09-15). "received" сам по себе больше не считается завершением:
  // партия может сколько угодно ждать в этом статусе, пока пользователь не
  // нажмёт "Завершить партию" (assertCanComplete, только из "received").
  // ОТК (production_order_qc_results) остаётся отдельным независимым фактом
  // и НЕ переводит статус сама по себе — ни в этот, ни в какой-либо другой.
  "completed",
  "cancelled",
]);
