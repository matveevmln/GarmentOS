...
function statusForCode(code: string): number {
  ...
  if (
    ...
    // Оптимистичная блокировка автосохранения заказа на пошив (ADR 0002) —
    // конфликт двух вкладок/устройств, тот же класс, что и остальные строки
    // выше: текущее состояние сервера не совпадает с ожидаемым клиентом.
