...
describe("Warehouse API (e2e)", () => {
  ...
  it("проходит полный цикл: приёмка → резерв → списание → перемещение → отгрузка → инвентаризация", async () => {
    ...
    const productResponse = await request(httpServer)
      .post("/v1/products")
      .send({ companyId: company.id, name: "Худи Петроль", code: "HOODIE-PETROL-WH-E2E" })
      .expect(201);
    ...
    const originResponse = await request(httpServer)
      .post("/v1/warehouses")
      .send({ companyId: company.id, name: "WIP Цех №1", type: "own" })
      .expect(201);
    const origin = originResponse.body as WarehouseResponseDto;

    const destinationResponse = await request(httpServer)
      .post("/v1/warehouses")
      .send({ companyId: company.id, name: "Склад продаж (Москва)", type: "own" })
      .expect(201);
  ...
  it("отклоняет несогласованность типа склада и workshopId — 400", async () => {
    ...
    await request(httpServer)
      .post("/v1/warehouses")
      .send({ companyId: company.id, name: "Склад без workshopId", type: "workshop" })
      .expect(400);
  });
});
