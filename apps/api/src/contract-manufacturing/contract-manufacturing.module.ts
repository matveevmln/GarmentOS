...
import { SewingOrdersController } from "./sewing-orders.controller";
import {
...
import { companyNumberingProvider } from "./company-numbering.provider";
...
@Module({
  ...
  controllers: [WorkshopsController, ProductionOrdersController, SewingOrdersController],
  providers: [
    ContractManufacturingService,
    bomApprovalProvider,
    companyNumberingProvider,
    ...
    {
      provide: SEWING_ORDER_REPOSITORY,
      useFactory: (db: Database) => new DrizzleSewingOrderRepository(db),
      inject: [DATABASE_CONNECTION],
    },
  ],
