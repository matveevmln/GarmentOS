...
| **Catalog** | Коллекции, модели одежды, SKU-матрица (размер × цвет) | `Collection`, `Product`, `ProductVariant` |
| **Materials & Procurement** | Материалы (вкл. упаковку), категоризированные поставщики, закупки, история цен (read-model) | `Material`, `Supplier`, `PurchaseOrder` |
...
| **Warehouse & Inventory** | Остатки, приёмки, отгрузки, инвентаризации, отгрузки/экспорт между складами (`DATABASE_SCHEMA.md` п.10) | `Warehouse`, `StockItem`, `StockMovement`, `Shipment`, `ShipmentItem` |
...
| **Finance** | Себестоимость (материалы + услуги цеха + логистика), движение денег, документы; прибыль — read-model, не таблица | `CostEntry`, `Transaction`, `Invoice` |
| **Notifications** | Уведомления пользователям (низкий остаток, срыв сроков и т.д.) | `Notification`, `NotificationRule` |
| **Reporting/BI** | Агрегированная аналитика (только чтение из других модулей), включая историю закупочных цен и прибыль | материализованные представления / read-models |
| **Inbox** (добавлен 2026-07-24 по итогам `docs/USER_JOURNEY_AUDIT.md`) | Универсальная точка входа для входящей информации (Telegram, файлы, голос) — AI классифицирует и предлагает вызов use case'ов других модулей; сам не хранит доменное состояние | `InboxChannel`, `InboxItem`, `InboxSuggestion` |

**Сквозные вспомогательные сущности** (используются несколькими модулями, не образуют отдельный bounded context): `Document` (файл) + `DocumentLink` (связь документа с сущностью, многие-ко-многим — один документ может относиться к модели, материалу, поставщику и закупке одновременно, `docs/PRINCIPLES.md` принцип 18) и `Note` — одна-к-одному, комментарий в контексте конкретной сущности (`DATABASE_SCHEMA.md` пп.15-16).

Полная архитектура модуля Inbox (pipeline классификации, каналы, границы и риски AI) — отдельный документ [`INBOX_ARCHITECTURE.md`](./INBOX_ARCHITECTURE.md), не дублируется здесь.

Полное описание таблиц каждого модуля — `DATABASE_SCHEMA.md`.

### Правило межмодульного взаимодействия

**Решено `docs/adr/0001-synchronous-orchestration.md` (2026-08-02)**: модуль A вызывает **application service** модуля B напрямую (in-process, через DI, внутри одной БД-транзакции), а не публикует событие — так уже реализовано и покрыто e2e-тестами для `Procurement → Warehouse` и `Contract Manufacturing → Warehouse`. Событийная шина не строится сейчас (один процесс, одна БД, два пользователя — усложнение без выгоды на этом масштабе), но каждый вызывающий сервис проектируется так, чтобы прямой вызов можно было заменить публикацией доменного события позже **без переписывания бизнес-логики** — оркестровка живёт в application-слое (не в `packages/domain/*`), вызывающий код обращается только к интерфейсу вызываемого модуля, не к его таблицам. Условие перехода на события — не календарная дата, а конкретный сигнал (модуль стал узким местом, нужно несколько независимых подписчиков на одно событие, модуль выделяется в отдельный сервис; см. п.10 ниже).

- Модуль A может вызвать application service модуля B, но не может читать/писать таблицы модуля B напрямую.
- Пример: `Sales` при подтверждении заказа вызывает `Warehouse.reserveStock()`, `HonestSign.retireCode()`, `Finance.recordCostEntry()` — последовательно, синхронно, в одной транзакции, тем же способом, что уже работает для закупок и заказов пошива (не через событие `StockReserved` — такого механизма в системе нет и не планируется строиться заранее).
...
sequenceDiagram
    ...
    WH-->>SALES: результат резерва (прямой вызов, не событие — ADR 0001)
...
> Мы не производим — мы размещаем заказ у независимого швейного цеха (`workshops`) и контролируем исполнение. Этапы кроя/пошива/ОТК происходят внутри цеха и нам операционно не видны — мы отслеживаем факт и срок исполнения заказа, а не внутренние стадии (см. `DATABASE_SCHEMA.md` п.0 и п.8).
...
Полная цепочка **от коллекции до прибыли** (включая закупку по категориям поставщиков, экспорт/отгрузку между складами и документооборот по заказу) — отдельная диаграмма в `DATABASE_SCHEMA.md`, раздел 3 («Жизненный цикл товара»), вместе с таблицей проверки, что каждый шаг реального бизнес-процесса отражён конкретной таблицей.
...
- **AuthZ**: RBAC через набор из 23 permissions (не жёстко закодированные роли — роли изменяемы без изменения кода), 7 предустановленных ролей (owner/director/accountant/procurement_manager/marketplace_manager/warehouse_keeper/viewer), реализовано в Итерации 5 — подробности `docs/AUTH_ARCHITECTURE.md`.
...
- **Аудит** (реализовано в Итерации 6, `packages/domain/audit`): критические доменные операции (изменение остатков, списание кодов маркировки, финансовые проводки, управление пользователями) пишут запись в `audit_log` с указанием пользователя (`user_id`, `null` для CLI-операций без человека-инициатора), времени и предыдущего/итогового состояния (`before_json`/`after_json`). Дополнительно фиксируется источник вызова — `source`: `http_api`/`cli`/`telegram`/`ai` — единый журнал независимо от того, кто инициировал действие (человек через веб, CLI-скрипт, будущий Telegram-бот или AI, подтверждённый человеком через Inbox). Для AI-подтверждённых действий — nullable `inbox_suggestion_id`, что именно предложил AI (`docs/AUTH_ARCHITECTURE.md`, раздел 13).
...
**Обновлено 2026-07-24**: первая версия этого раздела откладывала любую AI-функциональность за пределы Фазы 1. Это изменилось — модуль `Inbox` (раздел 3, [`INBOX_ARCHITECTURE.md`](./INBOX_ARCHITECTURE.md)) реализует AI-классификацию входящих сообщений уже в Фазе 1, потому что это оказалось прямой и приоритетной пользой для пользователя (`docs/USER_JOURNEY_AUDIT.md`), а не абстрактной задачей на будущее. Принцип из `docs/PRINCIPLES.md` (№15) при этом не изменился по сути: `Inbox`-классификатор — первый реальный AI-агент в системе — работает **строго через те же application services, что и человек**, никогда не пишет в доменные таблицы напрямую (см. `INBOX_ARCHITECTURE.md`, раздел 1). Остальные модули по-прежнему проектируются так, чтобы будущий AI-агент мог работать с системой теми же средствами, что и человек, без отдельного привилегированного пути:
