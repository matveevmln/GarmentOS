...
**Сквозные вспомогательные сущности** (используются несколькими модулями, не образуют отдельный bounded context): `Document` и `Note` — полиморфные (`entity_type`/`entity_id`) сущности для файлов (инвойсы, накладные, фото, сертификаты) и текстовых заметок по заказам/отгрузкам/закупкам (`DATABASE_SCHEMA.md` п.15).
