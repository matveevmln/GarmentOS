...
| `sewing_orders` | `id`, `company_id`, `number NULL` (выдаётся при размещении, `companies.next_sewing_order_number`), `workshop_id NULL` (обязателен при размещении), `status` (`draft/placed/cancelled`), `client_request_id NULL` (идемпотентность повторного размещения), `version` (optimistic locking автосохранения), `created_by`, `created_at`, `updated_at` | **Добавлено ADR 0002 (2026-09-24, T01).** Шапка заказа на пошив — «один цех, одна или несколько моделей», размещается атомарно одним действием. `production_orders` (одна строка на модель заказа) ссылаются на неё через `sewing_order_id` |
| `production_orders` | `id`, `company_id`, `product_id`, `bom_id`, `workshop_id`, `sewing_order_id NULL` (FK `sewing_orders`, ADR 0002), `planned_quantity`, `agreed_unit_price`, `materials_provided_by_us` (bool), `status` (`draft/placed/in_progress/ready_for_pickup/received/cancelled`), `due_date`, `received_at NULL` | Заказ пошива у конкретного цеха по согласованной цене за единицу — одна модель внутри `sewing_order`. `status=draft` — создан автоматически Inbox, ещё не подтверждён (п.0d); `status` в остальном — простое поле вместо отдельной таблицы стадий. `received_at` — фактическая дата завершения, без которой невозможно сравнить план (`due_date`) и факт для рейтинга цеха и алертов о просрочке (`USER_JOURNEY_AUDIT.md`, пробел №5) |
...
| `production_order_defects` | `id`, `company_id`, `production_order_id`, `qc_result_id NULL` (FK `production_order_qc_results`), `product_variant_id NULL` (NULL — общий брак без разбивки по SKU), `quantity` (>0), `reason NULL`, `created_by` | Факт брака, привязанный к конкретной партии (ПРОМПТ №10.2, этап B) — структурированная замена одинокого числа `production_order_qc_results.defect_quantity` |
| `production_order_defect_compensations` | `id`, `company_id`, `defect_id` (FK `production_order_defects`), `compensating_production_order_id` (FK `production_orders`), `compensating_variant_id NULL` (FK `production_order_variants`), `quantity` (>0), `created_by`, `created_at` | Явная связь «этот дефект закрыт вот этой строкой вот этого нового заказа» — создаётся только явным подтверждением пользователя (use case `createDefectCompensation`), никогда не выводится автоматически из одного лишь `source_production_order_id`/`variant_type='rework'` на новом заказе |
...
| `numeric_value_presets` | `id`, `company_id`, `kind` (`sewing_cost`/`specification_price`), `value`, `currency`, `created_by NULL`, `created_at` | Сохранённые числовые значения (ПРОМПТ №2/№3, раздел 5/11) — «стоимость пошива» и «цена спецификации» выбираются из списка ранее использованных значений компании, а не вводятся свободным текстом каждый раз; новое значение сохраняется для следующего раза. Append-only (нет `updated_at`/`deleted_at`) — значения не редактируются и не удаляются. Уникальность `(company_id, kind, value, currency)` — повторный ввод того же значения не плодит дубликат |
