...
## Структура `apps/prototype/`

Кликабельный визуальный прототип мобильного интерфейса (владелец проекта,
2 августа 2026) — статичная страница на реальных данных проекта, без единого
запроса к `apps/api`. Назначение — UX/UI-обзор перед началом `apps/web`, не
рабочий код: `index.html` (весь UI, включая клиентскую навигацию) + минимальная
Vite-обвязка (`pnpm dev`/`pnpm build`), чтобы владелец мог самостоятельно
подключить репозиторий к Vercel (Root Directory `apps/prototype`) и получать
Preview Deployment на каждый коммит без передачи токенов. После утверждения
направления UI этот каталог не развивается дальше — его сменяет `apps/web`.
